...
export const verifyRefs = (schema, file = "schema") => {
	const walk = (node) => {
		if (Array.isArray(node)) {
		...
		for (const [k, v] of Object.entries(node)) {
			if (k === "$ref" && typeof v === "string") {
				for (const [prefix, container] of Object.entries(
					LOCAL_REF_CONTAINERS,
				)) {
					if (!v.startsWith(prefix)) continue;
					const name = v.slice(prefix.length);
					const bag = schema[container];
					const resolves =
						bag &&
						typeof bag === "object" &&
						!Array.isArray(bag) &&
					...
					if (!resolves) {
						throw new Error(
							`${file}: dangling $ref "${v}" does not resolve to an entry in "${container}"`,
						);
					}
				}
			} else {
...
const build = () => {
	...
	for (const draft of drafts) {
		...
		verifyRefs(manifest, `${draft}.json`);
	...
	try {
		...
		console.error(`biome format step failed: ${err.message}`);
		throw err;
	}
};
